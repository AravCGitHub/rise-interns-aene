...
def graphObjandTimeVsImps(objArr, timeArr, numImpsArr):
    ...
    axis[0].plot(numImpsArr, objArrCVX, c='red', label="CVXOpt")
    axis[0].plot(numImpsArr, objArrAlg1, c='green', label='Alg1')
    axis[0].plot(numImpsArr, objArrAlg2, c='blue', label='Alg2')
    axis[1].plot(numImpsArr, timeArrCVX, c='red', label='CVXOpt')
    axis[1].plot(numImpsArr, timeArrAlg1, c='green', label='Alg1')
    axis[1].plot(numImpsArr, timeArrAlg2, c='blue', label='Alg2')

    # Labels
    axis[0].set_xlabel('Number of Advertisers')
    axis[0].set_ylabel('Objective Value')
    axis[1].set_xlabel('Number of Advertisers')
    axis[1].set_ylabel('Time Taken')
    axis[0].legend()
    axis[1].legend()
    plt.tight_layout()
    # Save and Clear
    plt.savefig('Images/New.png')
    plt.cla()
    plt.clf()
    plt.close()
